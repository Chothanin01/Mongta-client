...
class _EyeScanPageState extends State<EyeScanPage> with WidgetsBindingObserver {
  ...
  Widget _buildFullScreenCameraPreview() {
    ...
    // Camera preview for Samsung Fold and newer devices
    if (_cameraService.controller?.value.isInitialized ?? false) {
      ...
      return Stack(
        children: [
          // Full-screen camera preview with properly bounded constraints
          SizedBox.expand(
            child: Builder(
              builder: (context) {
                // Get screen dimensions to ensure proper constraints
                final size = MediaQuery.of(context).size;
                
                // Fix for Samsung Fold - avoid using AspectRatio directly
                return FittedBox(
                  fit: BoxFit.cover,
                  child: SizedBox(
                    width: size.width,
